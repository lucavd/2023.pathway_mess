...
query_to_pathwaysGO <- function(x) {
  ...
  tryCatch({
    ...
    GO_res <- enrichGO(gene = annot_als$Gene, OrgDb = org.Hs.eg.db , 
                       ...
                       pvalueCutoff = 0.01, pAdjustMethod = "holm", qvalueCutoff = 0.01,
...
write.csv(final_results_dfGO, "output/final_results_dfGO_holm.csv")

save(final_results_dfGO, file = 'output/final_resutsGO_holm.rda')
